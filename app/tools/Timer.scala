    ...
    class Task extends Actor {
      ...
      def receive = {
    	case CurrentTour(id,date)=> {
    	    ...
    	    for(user : User <- Tour.getAllUsersFor(id)){
                //TODO implement the following if notification is ready
                //Notification test = getInAdvanceNotification()
                //Mail.send(test)
                //SMS.send(test)
                //send E-Mail
                Mail.sendInAdvanceMail(user, actualTour)
                //send sms
                //TODO delete debug-flag
                SMS.sendTourNotification(user, actualTour, true)
            }
